# learning from https://github.com/jxgu1016/Total_Variation_Loss.pytorch
# we modify the pow2 to abs
...
class TVLoss(nn.Module):
    ...
    def forward(self,x):
        ...
        # h_tv = torch.pow((x[:,:,1:,:]-x[:,:,:h_x-1,:]),2).sum()  # we modify the pow2 to abs
        # w_tv = torch.pow((x[:,:,:,1:]-x[:,:,:,:w_x-1]),2).sum()  # we modify the pow2 to abs
        h_tv = torch.abs(x[:,:,1:,:]-x[:,:,:h_x-1,:]).sum()  # we modify the pow2 to abs
        w_tv = torch.abs(x[:,:,:,1:]-x[:,:,:,:w_x-1]).sum()  # we modify the pow2 to abs
