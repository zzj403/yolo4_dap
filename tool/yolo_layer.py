...
def yolo_forward_dynamic_attack(output, conf_thresh, num_classes, anchors, num_anchors, scale_x_y, only_objectness=1,
                              validation=False):
    # Output would be invalid if it does not satisfy this assert
    # assert (output.size(1) == (5 + num_classes) * num_anchors)
...
class YoloLayer(nn.Module):
    ...
    def forward(self, output, target=None):
        ...
        return yolo_forward_dynamic_attack(output, self.thresh, self.num_classes, masked_anchors, len(self.anchor_mask),scale_x_y=self.scale_x_y)
